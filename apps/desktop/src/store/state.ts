...
/** "applied" means an external edit was just auto-applied; previousContent is what to restore on Undo. */
...
export function applyFileAction(
	...
	// R15: files doc-history doesn't track (non-Markdown files) always get
	// a silent swap, no Undo pill.
	const isVersionableMarkdownFile = options?.isVersionableMarkdownFile ?? true;
	switch (action) {
		case "none":
			return state;
		case "match":
			return { ...state, ...cleanFileState(diskContent) };
		case "reload":
		case "conflict":
			...
			return {
				...
				externalChange: { kind: "applied", previousContent: state.content },
			};
	}
}
