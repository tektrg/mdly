...
describe("discoverWorkspaceFiles", () => {
	...
	// R15 — a single unreadable path mid-walk must not abort the rest of the
	// workspace. These two cases prove the engine itself (not just the
	// walkers built on top of it) is resilient.
	it("keeps walking past a directory it cannot read, and reports it separately", async () => {
		await writeFixture("visible/keep.md", "kept");
		...
		try {
			...
			).toEqual([path.join("visible", "keep.md")]);

			if (process.getuid?.() === 0) {
				// Running as root (common in CI containers) bypasses the
				// permission bits entirely, so "locked" is actually readable —
				// this environment cannot exercise the EACCES path at all.
				return;
			}
			expect(discovery.errors.some((entry) => entry.path === lockedDir)).toBe(
				true,
			);
		} finally {
	...
	it("keeps walking past a broken symlink without throwing", async () => {
		await writeFixture("visible/keep.md", "kept");
		await fs.symlink(
			"does-not-exist.md",
			path.join(workspaceRoot, "broken.md"),
		);
		...
		expect(
			discovery.files.map((entry) => path.relative(workspaceRoot, entry.path)),
		).toEqual(expect.arrayContaining([path.join("visible", "keep.md")]));
